...
const CartSidebar: React.FC<CartSidebarProps> = ({ isOpen, onClose, isSpecial }) => {
  ...
  const handleExportPDF = () => {
    ...
    if (name !== null) {
      setClientName(name);
      generatePDF(cart, name, cartTotal);
    }
  };

  const handleWhatsApp = () => {
    if (cart.length === 0) return;
    const url = generateWhatsAppLink(cart, cartTotal);
          ...
          cart.map((item) => (
            <div key={item.id} className="flex justify-between items-start bg-gray-50 p-3 rounded-lg border border-gray-100 group">
              <div className="flex-1 pr-2">
                ...
                <div className="text-alfonsa font-bold text-sm">
      ...
      <div className="p-4 bg-white border-t border-gray-200">
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
           <input 
             ...
             className="col-span-2 border border-gray-300 rounded px-3 py-2 text-sm focus:ring-1 focus:ring-alfonsa outline-none"
             value={clientName}
